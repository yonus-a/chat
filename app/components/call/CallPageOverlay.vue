<template>
  <div v-show="isReady" class="flex flex-col w-full h-full bg-diamond-black">
    <div class="h-16 sm:h-20 flex items-center justify-between px-4">
      <div class="md:block hidden text-white select-none text-label-lg">
        {{ t("chat.call.title") }}
      </div>
      <div
        class="bg-black-500 md:hidden rounded-full flex items-center gap-x-4 h-10 px-3"
      >
    ...
    >
      <CallBoard>
        <template #trigger="{ isOpen }">
          <div
            class="w-9 sm:w-12 transition-all duration-200 aspect-square rounded-full flex items-center justify-center"
      ...
      <div
        class="w-9 sm:w-12 transition-all duration-200 ease-in-out aspect-square rounded-full flex items-center justify-center"
        :class="[
          option.isActive
            ? ' bg-white cursor-pointer'
            : option.hasErrors
              ? 'bg-error-900 cursor-not-allowed'
...
export default defineComponent({
  ...
  setup() {
    ...
    const boardIcon = computed(() =>
      isBoardOpen.value ? "PhX" : "PhPencilCircle",
    );
    ...
    const initPermissions = async () => {
      ...
      const service =
        callStore.chatContact.serviceType === "video-call"
          ? "video-call"
          : "voice-call";

<template>
  ...
  >
    ...
    <video
      ...
      :class="{
        'scale-x-[-1]':
          ...
          targetMember.id === profileStore.userData.id,
      }"
    ></video>
    ...
    <div
      v-if="!showVideo"
      class="relative pointer-events-none select-none z-10 w-full h-full flex flex-col items-center justify-center gap-y-4 bg-black/20"
    >
      ...
      <div
        class="text-center text-white select-none text-label-md drop-shadow-md"
      >
    ...
    <!-- Action Layer -->
    <div
      class="p-2 w-full absolute h-full z-10000 flex items-end pointer-events-none"
    >
      <div
        @click="maximizeCall"
...
onMounted(() => {
  supportsNativePiP.value =
    typeof document !== "undefined" && "pictureInPictureEnabled" in document;
});

const targetMember = computed<Contact>(() => {
  const others = callStore.callMembers.filter(
    (m) => m.id !== profileStore.userData.id,
  );
  return (
    others[0] ||
    callStore.callMembers[0] || {
      id: profileStore.userData.id,
      name: profileStore.userData.name,
      lastName: profileStore.userData.lastName,
      imageUrl: profileStore.userData.imageUrl,
      isOnline: true,
      lastSeen: new Date(),
      isActive: false,
      unreadCount: 0,
...
const showVideo = computed(() => {
  ...
  const otherStreaming = callStore.callMembers.some(
    (m) => m.id !== profileStore.userData.id && m.stream,
  );
...
watch(
  () => [callStore.isPiP, callStore.localStream, callStore.isSharingScreen],
  () => {
    if (callStore.isPiP) {
      nextTick(() => updateStream());
    }

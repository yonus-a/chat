...
import { useI18n, useDate, useProfileStore, useChatStore } from "#imports";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const openChat = () => {
      chatStore.setActiveConversation(props.contact.id);
      chatStore.closeProfile();
    };

    const isFromMe = computed(
      () => props.contact.lastMessage?.senderId === profileStore.userData.id,
    );
    ...
    const lastMessageColor = computed(() => {
      ...
      if ((props.contact.unreadCount || 0) > 0)

  ...
  <div class="relative z-20 w-full">
    ...
    >
      ...
      >
        <div
          @click="openProfile"
          class="cursor-pointer flex items-center gap-x-3"
        >
          <div class="w-10 h-10 relative shrink-0">
            <ContactAvatar v-if="contact" :contact="contact" />
          </div>
          <div class="select-none">
            <div class="text-on-surface text-label-md">
        ...
        <div class="z-1001 relative h-6">
          ...
          >
            <div
              class="flex relative items-center gap-x-4 transition-all duration-200 ease-in-out"
              :class="[
                isSelectMode
                ...
                <BIcon
                  ...
                  :class="[
                    contact?.isActive || isInCall
                      ...
                      : 'fill-on-surface/25 cursor-not-allowed',
                  ]"
                />
              </div>
            ...
            >
              ...
              <BIcon
                @click="copy"
                icon="PhCopy"
                class="w-6 h-6 cursor-pointer fill-on-surface"
              />
            </div>
          </div>
        </div>
      ...
      <BIcon
        @click="goBack"
        icon="PhArrowLeft"
        class="md:hidden fill-on-surface/50 w-6 h-6 cursor-pointer"
      />
    </div>
    <div class="w-full absolute bottom-0 z-20 h-0 overflow-visible">
      <div
        class="w-full flex items-center transition-all duration-200 ease-in-out overflow-hidden whitespace-nowrap text-wrap px-2 bg-diamond-primary-secondary"
        :class="[callData.show ? ' h-11' : 'h-0']"
      >
        <div
          class="w-full flex items-center gap-x-3 transition-all duration-200 ease-in-out"
          :class="[callData.show ? 'opacity-100' : 'opacity-0']"
        >
          <BIcon class="fill-white w-5 shrink-0 h-5" icon="PhPhoneCall" />
          <div class="flex-1 text-white select-none text-label-md">
            {{ callData.duration }}
          </div>
          <BIcon
            class="fill-white cursor-pointer w-5 shrink-0 h-5"
            @click="backToCall"
            icon="PhFrameCorners"
          />
        </div>
...
export default defineComponent({
  ...
  emits: ["call", "open-profile", "go-back"],
  setup(props, { emit }) {
    ...
    const currentConversationId = computed(() => props.conversationId);
        ...
        case "prescribe-meds":
          if (currentConversationId.value) {
            if (
              ...
              chatActionStore.triggerPersonalInfoRequest(
    ...
    const initCall = () => {
      if (props.contact?.isActive) {
        const serviceType =
          props.contact.serviceType === "video-call"
            ? "video-call"
            : "voice-call";
        callStore.startCall(props.contact, serviceType);
      }
    };

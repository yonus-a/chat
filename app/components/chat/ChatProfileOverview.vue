<template>
  <div
    class="transition-none md:transition-all duration-300 ease-in-out h-full overflow-hidden rtl:border-l-surface-variant ltr:border-surface-variant ltr:border-r rtl:border-l bg-surface shrink-0"
    ...
    <div class="w-full h-full flex flex-col">
      <div class="flex flex-col md:pt-16.5 md:px-2 w-full h-full">
        <div class="shrink-0 w-full relative">
          <div class="w-full h-29">
            ...
            >
              <div class="w-full h-full p-2">
                <BIcon
                  icon="PhX"
                  class="fill-white w-5 h-5 cursor-pointer"
                  @click="closeSidebar"
                />
              </div>
            </BImage>
          ...
          <div
            class="w-full absolute z-20 flex justify-center items-center -translate-y-1/2"
          >
        ...
        <div
          class="shrink-0 w-full mt-2 flex flex-col items-center select-none justify-center gap-y-2"
        >
          ...
          <div class="w-full py-4 flex items-center justify-center gap-x-2">
            ...
            >
              <BIcon
                ...
                weight="fill"
              />
              <div class="text-center text-on-surface text-[10px] select-none">
                {{ action.title }}
              </div>
            </div>
          </div>
        ...
        <div class="w-full flex-1 flex flex-col min-h-0 select-none">
          <div
            v-if="!shouldShowTabs"
            class="flex min-h-0 flex-col gap-y-1 w-full h-full"
          >
            <div
              v-if="fileAttachements.length > 0"
              class="w-full shrink-0 flex flex-col gap-y-1"
            >
            ...
            <div
              ...
              class="flex-1 flex-col gap-y-1"
            >
              <div class="shrink-0 text-on-surface/50 text-body-sm">
                {{ t("chat.info.media") }}
              </div>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const localProfile = ref<Contact>(mockProfile.value);
    ...
    const actionButtons = computed((): Action[] => [
      ...
      },
      {
        ...
        icon: "PhPhoneCall",
        active:
          localProfile.value == undefined || isInCall.value
            ? false
            : localProfile.value?.isActive &&
    ...
    const fileAttachements = ref<string[]>(
      new Array(imagesPerPage.value).fill(""),
    );
    const mediaAttachements = ref<string[]>(
      new Array(filesPerPage.value).fill(""),
    );

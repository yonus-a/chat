<template>
    <div class=" w-full flex items-center gap-x-2">
        ...
        <div class=" flex-1 text-on-surface">
            <div class="text-label-md" v-loading="isLoading">{{ `${provider.name} ${provider.lastName}` }}</div>
            <div class="text-body-sm max-w-full text-ellipsis overflow-hidden line-clamp-1 opacity-50"
                v-loading="isLoading">{{ fellowshipString }}
            </div>
        </div>
        <div class=" shrink-0 select-none text-on-surface text-label-md ">
            <div class="flex items-center gap-x-2" v-if="!isPending">
                ...
                <div v-loading="">{{ t('general.currency') }}</div>
            </div>
            <div v-else class=" flex items-center gap-x-1">
                <BIcon icon="PhClock" class=" w-4 h-4 fill-primary" />
                <div class=" text-primary text-[10px]">{{ t('chat.requestCard.addMedic.pending') }}</div>
            </div>
        </div>
    </div>
</template>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const fellowshipString = computed(() => {
            if (!provider.value.fellowships || provider.value.fellowships.length === 0) {

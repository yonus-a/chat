<template>
  <div
    class="p-3 w-80 flex flex-col gap-y-3 select-none bg-surface rounded-2xl shadow-floating"
  >
    <div class="w-full flex flex-col gap-y-3 h-full" v-if="isServiceRequest">
      <div
        class="transition-all duration-200 ease-in-out select-none text-label-sm"
        :class="[isPending ? 'text-on-surface/50' : 'text-on-surface']"
      >
        {{ t("chat.addMedic.title") }}
      </div>
      ...
      <div
        v-else
        class="h-14 w-full rounded-full flex items-center justify-center gap-x-2"
      >
        <div
          class="shrink-0 w-10 h-10 rounded-full bg-surface-variant flex items-center justify-center"
        >
          <BIcon icon="PhIdentificationCard" class="fill-primary w-6 h-6" />
        </div>
        <div
          class="flex-1 flex-col h-full flex select-none text-on-surface justify-center"
        >
          <div class="text-label-md" v-loading="isPending">
            <div class="h-7 w-30"></div>
          </div>
          <div class="text-body-sm opacity-50">
            {{ request?.request.service.label }}
          </div>
        </div>
        <div class="h-7 w-20" v-loading="isPending"></div>
      </div>
      <div
        class="w-full text-center select-none text-label-sm"
        :class="[isCanceled ? 'text-error' : 'text-primary']"
      >
        {{ cardSubText }}
      </div>
    </div>
    <div
      class="w-full flex flex-col justify-center gap-y-3 items-center"
      v-else
    >
      <div
        v-if="infoAccessContent && request?.request.status == 'pending'"
      ...
      <div
        v-else-if="infoAccessContent"
        class="w-full flex flex-col gap-y-3 items-center select-none justify-center py-2"
      >
        <BIcon
          weight="fill"
          :icon="infoAccessContent.icon"
          :class="[`fill-${infoAccessContent.color}`]"
          class="w-14 h-14"
        />
        <div
          :class="[`text-${infoAccessContent.color}`]"
          class="text-label-md font-medium text-center"
        >
    ...
    <div class="w-full flex items-center gap-x-3">
      <BButton
        ...
        v-for="button in requestButtonProps"
        :key="button.key"
        :disabled="
          button.disabled || !!chatActionStore.processingActions.get(message.id)
        "
    ...
    <div
      ...
      class="gap-x-3 w-full flex items-center justify-center cursor-pointer"
      v-if="
        isMedic &&
        message.request?.type === 'add-person' &&
        message.request.request.status === 'approved'
      "
    >
      <div class="text-on-surface text-label-lg select-none">
        {{ contact.nationalCode }}
      </div>
      <BIcon
        :icon="isCopied ? 'PhCheckCircle' : 'PhCopy'"
        class="w-4.5 h-4.5 fill-on-surface/50"
      />
    </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const isServiceRequest = computed(
      () => request.value?.type === "add-person",
    );
    ...
    const isPending = computed(() => {
      if (request.value?.type === "add-person") {
        ...
        return (
          !finalKeys.includes(request.value.request.status) &&
          providers &&
          providers.length !== 1
        );
      } else {
    ...
    const isMedic = computed(
      () =>
        profileStore.userData.id == props.message.senderId &&
        profileStore.chosenRole !== "user",
    );
    ...
    const cardSubText = computed(() => {
      ...
      if (request.value?.type === "add-person") {
        switch (status) {
          case "payment":
            text = isMedic.value
              ? t("chat.requestCard.addMedic.awaitingPayment")
              : t("chat.requestCard.addMedic.awaitingPatientPayment");
            break;
          case "searching":
          ...
          case "pending":
            text = !isMedic.value
              ? t("chat.requestCard.addMedic.awaitingApproval")
              : t("chat.requestCard.addMedic.awaitingMedicApproval");
            break;
          case "rejected":
    ...
    const isCanceled = computed(() => {
      ...
      return (
        request.value?.request.status === "expired" ||
        request.value?.request.status === "rejected"
      );
    });

    const messageId = computed(() => props.message.id);
    const chatId = computed(() => chatStore.activeConversationId || 0);

    const requestButtonProps = computed(() => {
      const isBusy = (key: string) =>
        chatActionStore.isActionBusy(messageId.value, key);
      if (request.value?.type === "add-person") {
          ...
          return [
            {
              type: "fill",
              color: "secondary",
              text: t("chat.requestCard.addMedic.retry"),
              key: "resend-request",
            },
          ];
        }
            ...
            ? [
                {
                  type: "outline",
                  color: "error",
                  text: t("chat.requestCard.addMedic.cancel"),
                  key: "cancel-request",
            ...
            : [
                {
                  type: "fill",
                  color: "primary",
                  text: t("chat.requestCard.addMedic.confirmRequest"),
                  key: "approve-request-user",
                },
                {
                  type: "fill",
                  color: "secondary",
                  text: t("chat.requestCard.addMedic.reject"),
                  key: "reject-medic-request",
                },
              ];
        }
        ...
        if (status === "payment" && !isSelf) {
          ...
          const hasPaymentProvider = providersList.some(
            (p) => p.status === "payment",
          );
      ...
      switch (key) {
        ...
        case "reject-access":
          modal.value?.openModal(
            t("chat.requestCard.infoAccess.rejectModal.title"),
            t("chat.requestCard.infoAccess.rejectModal.description"),
            "error",
            true,
            t("chat.requestCard.infoAccess.rejectModal.reject"),
          );
          break;

<template>
    <div class="relative w-60.5 h-60.5 p-2 shrink-0 group select-none flex items-center justify-center">
        <svg v-if="mode === 'recording' || isUploading" class="absolute w-57.5 h-57.5 -rotate-90 pointer-events-none z-20" viewBox="0 0 230 230">
            <circle cx="115" cy="115" r="113" class="stroke-white/30" stroke-width="4" fill="none" />
            <circle cx="115" cy="115" r="113" class="stroke-primary transition-all ease-linear duration-200"
                stroke-width="4" fill="none" stroke-linecap="round" :stroke-dasharray="circumference" :stroke-dashoffset="dashOffset" />
        </svg>
        ...
        <div class="relative w-54.5 h-54.5 shrink-0 rounded-full overflow-hidden bg-[#1C1E22] cursor-pointer z-10"
            ...
            <div v-if="!isLoaded && mode === 'playback'" class="absolute inset-0 flex items-center justify-center bg-surface"></div>
            ...
            <div v-if="mode === 'recording'" class="absolute top-4 left-1/2 -translate-x-1/2 flex items-center justify-center gap-x-1">
...
export default defineComponent({
    ...
    props: {
        ...
        mode: { type: String as () => 'playback' | 'recording', default: 'playback' },
    ...
    setup(props) {
        ...
        const handleTimeUpdate = () => {
            ...
            if (total) progress.value = (current / total) * 100;
        };
        ...
        onBeforeUnmount(() => { if (videoRef.value) videoRef.value.srcObject = null; });

        return {
            videoRef, isPlaying, isLoaded, circumference, dashOffset,
            togglePlay, handleTimeUpdate, handleEnded, handlePlay, handlePause, isUploading
        };
    }
});
</script>
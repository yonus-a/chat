...
export default defineComponent({
  ...
  setup(props, { expose, emit }) {
    ...
    const recording = useChatRecording(inputWidth, {
      onStart: () => {
        if (secondaryMessageType.value === "video")
          videoDisplayRef.value?.open();
      },
      onCancel: () => {
        if (secondaryMessageType.value === "video")
          videoDisplayRef.value?.close();
      },
    ...
    const currentRecordingSeconds = computed(() => {
      ...
      const [minutes, seconds] = recording.formattedTime.value
        .split(":")
        .map(Number);
    ...
    const createBaseMessage = (): Message => {
      return {
        ...
        conversationId: props.conversationId || 101,
        date: new Date(),
        type: "text", // default
        isEdited: false,
    ...
    const sendMessage = () => {
      ...
      if (textMode.value === "edit" && editingMessageData.value) {
        chatActionStore.saveEditMessage(
    ...
    const handleEscapeNavigation = () => {
      if (callStore.isActive && !callStore.isPiP) {
    ...
    const handleEmojiSelect = (emoji: string) => {
      ...
      if (firstChunk && firstChunk.type === "emoji") {
        ...
        img.className =
          "inline-block w-5 h-5 mx-0.5 align-middle select-text pointer-events-none";
    ...
    const handleFlipCamera = () => {
      if (typeof (recording as any).toggleCamera === "function") {
        ...
        console.warn(
          "Camera flip requested, but toggleCamera method is missing in useChatRecording",
        );
      }
    };
    ...
    return {
      ...
      iconClass: computed(() =>
        !props.isActive
          ? "opacity-50 fill-on-surface pointer-events-none"
          : "fill-on-surface opacity-100 pointer-events-auto",
      ),

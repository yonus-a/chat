  ...
  <div class="max-w-dvw w-full lg:w-90 lg:p-5 flex items-center justify-center">
    <div class="w-full max-w-90">
      <div class="flex items-center gap-x-3">
        <BIcon
          icon="PhX"
          class="w-5 h-5 cursor-pointer fill-on-surface/50"
          @click="close"
        />
        <div class="text-on-surface text-label-sm select-none">
          {{ t("chat.addMedic.title") }}
        </div>
      </div>
      <BSelect
        v-model="field"
        :title="t('chat.addMedic.expertise')"
        :placeholder="t('chat.addMedic.select')"
        :loading="isLoadingServices"
        :options="serviceOptions"
        class="max-w-full min-w-full w-full pt-4"
      />
      <BCheckBox
        :disabled="isLoadingServices"
        v-model="autoSelect"
        :label="t('chat.addMedic.autoSelect')"
        mode="switch"
      />
      <div
        class="w-full transition-all duration-200 ease-in-out whitespace-nowrap overflow-hidden"
        :class="[
          autoSelect
            ? 'h-0 pt-0 opacity-0 pointer-events-none'
            : 'h-auto pt-4 opacity-100 pointer-events-auto',
        ]"
      >
        <BInput
          class="max-w-full min-w-full w-full"
        ...
        <div class="w-full h-90.5 mt-4 relative">
          <div v-if="isLoading || providers.length > 0" class="w-full h-full">
            <BVirtualVerticalList
              :items="providers"
              @load-more="serviceStore.fetchProviders(true)"
              :hasNextPage="serviceStore.hasProviderNextPage"
              :loading="isLoading"
            >
              <template #item="{ item: medic }">
                <div class="w-full pb-2">
                  <MedicDisplay
                    @click="toggleSelect(medic)"
                    :isSelected="isSelected(medic.id)"
                    :medic="medic"
          ...
          <div
            v-else-if="!isLoading && providers.length === 0"
            class="w-full h-full flex items-center justify-center"
          >
            <NoDataDisplay
              :image-path="NoProviderImage"
      ...
      <BButton
        ...
        class="transition-all duration-200 ease-in-out min-w-full"
        :class="[!autoSelect ? ' mt-0' : 'mt-4']"
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    const currentConversationId = computed(
      () => chatStore.activeConversationId || 0,
    );
    ...
    const isSelected = (id: number) =>
      selectedMedics.value.some((p) => p.id === id);

    const buttonProps = computed(() => {
      let buttonText = t("chat.addMedic.buttonText.single");
      let disabled =
        !autoSelect.value &&
        (selectedMedics.value.length > 5 || selectedMedics.value.length === 0);
      if (selectedMedics.value.length > 1) {
        buttonText = t("chat.addMedic.buttonText.multiple", {
          count: selectedMedics.value.length,
        });
      }
      return {
        disabled: disabled,

...
export function useChatRecording(
    ...
    const cancelOpacity = computed(() => dragAxis.value === 'y' ? 1 : 1 - Math.min(Math.abs(dragOffset.value.x) / 80, 1));
    ...
    return {
        isRecording, isLocked, isPaused, formattedTime, 
        dragOffset, isDragging, lockOpacity, cancelOpacity, isLongPress,
        onPointerDown, togglePause, stopRecording
    };
}